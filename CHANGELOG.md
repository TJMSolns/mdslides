...
### Fixed
- **Domain test alignment (MS-015):** 20 pre-existing domain test failures resolved. `HeaderFooter.resolvePlaceholders` now uses single-brace `{title}`, `{author}`, `{date}`, `{current-slide}`, `{total-slides}`, `{elapsed-time}` format matching the documented spec. Property-based and example tests for density validation updated to check for `DensityWarning` (per PDR-001) instead of `ContentError`.
