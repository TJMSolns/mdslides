...
**Sequenced (Tony, GL-030, 2026-07-10):** MS-018 → MS-019 → MS-017 (Bayesian-nudge rationale: MS-018 directly feeds open-agentic-platform's WQ-179 — nudged up; MS-019 completes the MCP surface, a stated precondition for "any new MCP work" — nudged up moderately; MS-017 is a valuable, self-contained bug-class fix with nothing else named depending on it). MS-018 (1 of 3) done 2026-07-10; MS-019 (2 of 3) done 2026-07-11 — MS-017 (3 of 3) opened its Pre-Implementation Gate 2026-07-11 (design note written, HL-022) and is now gated on Tony's review rather than blocked on sequence.

| MS-017 | **[Sequence: 3 of 3 — GL-030]** **P4** — Introduce typed slot name ADT (`sealed trait SlotName` with case objects for each slot: `Heading`, `Body`, `Code`, etc.) and enforce via the parser-renderer interface. Eliminates the class of silent slot-name mismatch bugs (LL-003: heading slot extracted but renderer looked for different key — no error, feature just disappeared). Source: retro-all RL-002. **Pre-Implementation Gate opened 2026-07-11:** `doc/internal/planning/design-MS-017-typed-slot-name.md` — found the WQ text's `sealed trait` conflicts with repo's `enum` convention, the slot-name keyspace is closed (not open/config-driven as ADR-008 anticipated), and `Slide.slots` is also used to carry unrelated frontmatter metadata (`header`/`footer`/`vertical-align`) — two design decisions (ADT shape, scope) recommended in the doc but left for Tony's review before implementation. | Claude | **Gated — awaiting Tony review of design note** | — |
