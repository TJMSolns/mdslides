...
**Sequenced (Tony, GL-030, 2026-07-10):** MS-018 → MS-019 → MS-017 (Bayesian-nudge rationale: MS-018 directly feeds open-agentic-platform's WQ-179 — nudged up; MS-019 completes the MCP surface, a stated precondition for "any new MCP work" — nudged up moderately; MS-017 is a valuable, self-contained bug-class fix with nothing else named depending on it). MS-018 (1 of 3) done 2026-07-10; MS-019 (2 of 3) done 2026-07-11 — MS-017 (3 of 3) opened its Pre-Implementation Gate 2026-07-11 (design note written, HL-022), was approved by Tony 2026-07-11, and is now fully implemented with an independent verifier PASS (3 rounds — see evidence) — held out of `## Done` only by MS-023 below, not by any remaining doubt about the work itself.

| MS-017 | **[Sequence: 3 of 3 — GL-030]** **P4** — Introduce typed slot name ADT and enforce via the parser-renderer interface. Eliminates the class of silent slot-name mismatch bugs (LL-003). Source: retro-all RL-002. **Implementation complete** (commits `a40c4db`, `697178b`, `1abd734`); `mill __.compile`/`mill __.test` clean (687/687); independent verifier **PASS** (sonnet tier, round 3 — round 1 VETO'd on an evidence-disclosure gap since fixed, round 2 PASS but not transcript-corroborated). **Held out of `## Done` by MS-023** (below): this project's own `pretooluse-done-gate.py` DN-006 transcript-corroboration check cannot recognize this session's async-notification format in this execution environment, so it hard-blocks the Done transition despite three real, independent verifier dispatches and a genuine final PASS. Not routed around — queued as MS-023 instead, per this project's inline-bug-discovery convention. Awaiting either a hook fix or Tony's explicit call on how to proceed. | Claude | **Implementation + verifier PASS complete — Done transition blocked by MS-023** | MS-023 |
| MS-023 | **BUG** — `pretooluse-done-gate.py`'s DN-006 transcript-corroboration check does not recognize this execution environment's async task-notification format. The hook's `iter_message_texts()` only reads `entry["message"]["content"]` blocks, but in this environment a completed Agent-tool sub-task's `<task-notification>` is delivered via a `{"type":"queue-operation","operation":"enqueue",...,"content":"<task-notification>..."}` transcript entry — a shape `iter_message_texts()` never inspects (no `"message"` key at all). Result: `transcript_corroborates_pass()` returns `False` (not `None`) for every genuinely-dispatched, genuinely-PASSing verifier call in this environment, since the transcript IS readable but the notification is structurally invisible to the parser — a hard, silent false-negative block, discovered while trying to mark MS-017 Done after 3 real independent verifier dispatches (final: PASS). Confirmed via direct `grep`/read of the session's own transcript JSONL. Fix (not attempted here — out of MS-017's scope and inappropriate for the doer of the gated item to alter the gate that checks its own work): extend `iter_message_texts()`/`parse_verifier_agent_calls()`/`find_notification_result()` to also read `"type":"queue-operation"` entries' top-level `"content"` string field, in addition to (not instead of) the existing `message.content` array path, so both transcript shapes corroborate. | Claude (discovered, not fixed) | Queued | — |
