...
| MS-020 | **P4 MAINTENANCE** [⬆ WQ-P4-092 (org)] — Execute theme/asset rename: update any `retisio`-keyed theme identifiers, asset filenames, and references in mdslides source and examples to `tjmsolutions`-prefixed equivalents. Run `mill __.compile && mill __.test` to validate; bump PATCH version if any CLI-visible strings change. Source: HL-060 / WQ-P4-092 deferred to mdslides session. **[Promoted Queued — GL-031, 2026-07-18: org source WQ-P4-092 confirmed Queued/unblocked, `Depends On: —`.]** **[SCOPE-BLOCKED — HL-061, 2026-07-18: promoted correctly, but NOT autonomously executable as written; needs 2 Tony decisions. (1) Namespace collision — the rename target is taken: `themes/tjm-solutions.json` AND `themes/tjm-solutions/theme.json` both already exist and are a *different* theme (red `#C00000`/Lato/no backgrounds vs. retisio's navy-gold-green `#002C74`/Varela Round/5 background PNGs + `templateConfig`). WQ-P4-092's text assumed a free target name; there isn't one. (2) The 5 background PNGs are RETISIO's own brand artwork (byte-identical, md5 `989ac398…`, duplicated across 5 dirs; `doc/internal/reference/tmp/` also holds 2 RETISIO logo files) — renaming them to `tjmsolutions-*` relabels a client's artwork as TJM Solutions' and is worse than the status quo. Correct disposition is probably DELETE (POL-001), which is not what "rename" authorises and is exactly the deck-breaking change WQ-P4-092 deferred over. Status left `Queued` deliberately (reclassifying is a `/groom` action). Fix is likely at org level in WQ-P4-092's own text.]** | Claude | Queued | — |
