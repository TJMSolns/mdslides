...
class FrameworkCopier(
  ...
  def copyFrameworkFiles(): Int = {
    ...
    // Copy mill-deploy-plugin/ (self-sufficiency)
    filesCopied += copyDirectory(
    ...
    // Copy mill-bootstrap-plugin/ (self-sufficiency)
    filesCopied += copyDirectory(
    ...
    filesToUpdate.foreach { file =>
      if (os.exists(file)) {
        ...
        val updated = content
          ...
          .replace("RETISIO/copilot-training", s"RETISIO/$projectName")
